...
```bash
schx-job-viewer$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
schx-job-viewer$ sam local start-api
schx-job-viewer$ curl http://localhost:3000/
```
...
```bash
schx-job-viewer$ sam logs -n HelloWorldFunction --stack-name schx-job-viewer --tail
```
...
```bash
schx-job-viewer$ cd hello-world
...
```bash
sam delete --stack-name schx-job-viewer
```
